...
function searchNext() {
    for (let i = 0; i < jsonData.length && idx < limit; i++) {
        let item = jsonData[i];
        console.log(item, idx);
        if (reg.test(item)) {
            document.getElementById('suggestionsBar').innerHTML += '<br>' + item;
            idx++;
        }
    }
    // if (idx >= jsonData.length || idx > limit) {
